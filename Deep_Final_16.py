...
def neural_network_model(data):
    ...
    print(tf.cast(hidden_l1['weights'], tf.float32))
    print(tf.cast(hidden_l1['biases'], tf.float32))
    ...
    l1 = tf.add(tf.matmul(tf.cast(data, tf.float32), tf.cast(hidden_l1['weights'], tf.float32)),
                tf.cast(hidden_l1['biases'], tf.float32))
    l1 = tf.nn.relu(l1)

    l2 = tf.add(tf.matmul(l1, tf.cast(hidden_l2['weights'], tf.float32)), tf.cast(hidden_l2['biases'], tf.float32))
    l2 = tf.nn.relu(l2)

    l3 = tf.add(tf.matmul(l2, tf.cast(hidden_l3['weights'], tf.float32)), tf.cast(hidden_l3['biases'], tf.float32))
    l3 = tf.nn.relu(l3)

    output = tf.add(tf.matmul(l3, tf.cast(output_l['weights'], tf.float32)), tf.cast(output_l['biases'], tf.float32))
    return output
